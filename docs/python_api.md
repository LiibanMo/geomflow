...
All public tensor operations preserve the input device and use float32 or
float64. Models, inputs, metric callback outputs, and atlas runtime state must
share a device and dtype; no low-level operation moves tensors implicitly.
CUDA float16, bfloat16, and autocast are unsupported. Exact divergence is the
default. Stochastic divergence must be requested explicitly and produces an
approximate likelihood.

Direct autograd is the default gradient path and is required for multi-chart
training. The single-chart intrinsic adjoint reduces trajectory storage but
uses prefix replay, supports only first-order derivatives, and can cost more
runtime. See `gpu_support_contract.md` and `gpu_troubleshooting.md`.
...
`batched_jacobian` treats every leading index as an independent sample and
returns `J[..., i, j] = partial_j f^i`. It flattens arbitrary leading batch
dimensions internally and restores them in the result. Metric callbacks with
an analytic `derivative_fn` avoid fallback differentiation and are preferred
in performance-sensitive code.

`divergence` computes exact intrinsic divergence by default. The explicit
`divergence_mode="hutchinson"` option uses one Rademacher probe per sample and
...
Geometry and transition callbacks must return tensors with the documented
shape on the same device and with the same dtype as their input. Derive
constants from the input, for example with `x.new_zeros(...)`, `torch.eye(...,
device=x.device, dtype=x.dtype)`, or a `*_like` operation. Structural checks
are always enabled. `AnalyticMetric(..., debug_validation=True)` additionally
checks finiteness and metric symmetry; this diagnostic mode may synchronize a
CUDA stream and should not be enabled in performance-sensitive loops.

## Device And Dtype Policy

Public PyTorch operations support `torch.float32` and `torch.float64` on CPU
and CUDA. Low-level geometry, vector-field, integration, and likelihood APIs
never move complete tensors implicitly. Inputs, model parameters, persistent
atlas samples, and callback results must already agree in device and dtype;
mismatches fail at the API boundary with the expected and actual placement.

`preprocess(data, device=..., dtype=...)` is the explicit input-conversion
boundary. Without `device`, tensor inputs retain their existing device.
`ManifoldCNF.fit` requires callers to place both model and data explicitly:

```python
model = model.to(device=device, dtype=dtype)
data = preprocess(data, device=device, dtype=dtype)
history = model.fit(data)
```

`ManifoldCNF.sample` uses the model's canonical parameter device and dtype.
Its optional `device` argument may confirm that placement but cannot request a
different device. Atlas sample tensors participate in module movement, dtype
conversion, and state dictionaries; chart identifiers remain Python integers
and domain masks remain boolean tensors.
