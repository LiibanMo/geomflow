...
The adjoint uses constant-memory deterministic prefix replay: custom forward
stores no trajectory states, while backward reconstructs each accepted RK step
and immediately applies its local vector-Jacobian product. Memory is independent
of the accepted step count, at the cost of quadratic recomputation. Stochastic
or state-mutating field evaluation is outside this contract. This API rejects
parameterized metrics and base distributions. Higher-order gradients through
custom backward are unsupported; direct `cnf_nll` remains the reference path
