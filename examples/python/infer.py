...
class CNFNet(torch.nn.Module):
    def __init__(self, dim: int = 2, hidden: int = 32):
        super().__init__()
        self.net = torch.nn.Sequential(
            torch.nn.Linear(dim + 1, hidden),
            torch.nn.Tanh(),
...
def main():
    ...
    if use_logp and log_densities[0] is not None:
        pc["_logp"] = log_densities[0]  # type: ignore[arg-type]
    else:
        pc["_z"] = init_pts[:, 2]

    scalar_name = "_logp" if use_logp else "_z"
    ...
    for i in range(len(frames_cart)):
        ...
        # update scalars
        if use_logp and log_densities[i] is not None:
